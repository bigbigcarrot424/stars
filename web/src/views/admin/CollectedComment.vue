<template>
    <a-comment>
        <template #actions>
            ...
            <span key="cancel-collect">
                  <a-popconfirm title="确定取消收藏？" ok-text="是" cancel-text="否" @confirm="cancelCollectBlog" @cancel="">
                    <StarFilled/>
                  </a-popconfirm>
            </span>
        </template>
...
<script lang="ts">
    ...
    import {LikeFilled, LikeOutlined, DislikeFilled, DislikeOutlined, StarOutlined, StarFilled} from '@ant-design/icons-vue';
    ...
    export default defineComponent({
        ...
        setup(props) {
            ...
            const cancelCollectBlog = () => {
                axios.get(SERVER + '/collect/delete/' + store.state.user.id + '/' + props.blogInfo.id).then((response) =>{
                    const data = response.data;
                    if (data.success){
